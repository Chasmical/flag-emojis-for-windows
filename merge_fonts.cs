...
foreach (var flagColor in flagsColors) {
    ...
    if (dissimilarColors.TryGetValue(flagColor, out var simColor)) {
	    ...
	    if (colorToId.TryGetValue(simColor, out var id)) {
		    ...
		    colorToId[flagColor] = flagsColors.IndexOf(simColor);
			Debug.Assert(flagsColors.Contains(simColor));
		    mapPaletteEntries[oldId] = flagsColors.IndexOf(simColor);
		}
	} else {
	    // otherwise, add to Segoe's <CPAL> with new id
		int newId = paletteCounter++;
		colorToId[flagColor] = newId;

		mapPaletteEntries[oldId] = newId;
    	tEntry.SetAttribute("index", newId.ToString());
	    var clone = sdoc.ImportNode(tEntry, deep: true);
